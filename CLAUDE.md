...
**Current status:** v2.0.0.0 ready to build — Store install failure fix: added microphone DeviceCapability (WebView2 needs explicit OS-level mic permission), internetClientServer capability, and scoped resources.pri generation to images/ only (full-root indexing with woff2/JS files caused corrupt .pri → "failed to install" on Surface Laptop). resources.pri now fails loudly if makepri.exe is missing. Also: mic interim-transcript in input field + 8s timeout; OpsMentor side-minimize with container resize; role display in footer.
