...
**Current status:** v2.3.0.0 ready to build — Crash at launch fix on Windows 11 24H2 26100.3194 (DELL OptiPlex 7000, Surface Laptop Go). Root cause: Vite 8 (rolldown) emits `crossorigin` on every `<script>`, `<link rel="modulepreload">`, and `<link rel="stylesheet">` tag. On WebView2 on 26100.3194, the `crossorigin` attribute causes the browser to apply CORS semantics to local `ms-appx-web:` resources, which crashes the WebView2 renderer natively (Error: N/A). Fix: (1) `build.modulePreload: false` in vite.config.js removes all `<link rel="modulepreload">` tags from the build output; (2) new MSIX workflow step strips any remaining `crossorigin` attributes from msix/index.html after copying dist/ — the main `<script>` and `<link rel="stylesheet">` tags still get `crossorigin` from Vite but are cleaned before packaging; (3) system font override injected by index.html inline script for ms-appx-web: context prevents @fontsource Inter woff2 files from being requested (belt-and-suspenders — @font-face loading could also cause issues on certain WebView2 builds); (4) `unlockAudio()` in OrchestratorPanel.jsx returns immediately in ms-appx-web: context — AudioContext creation in WebView2 ms-appx-web: scheme is an additional crash risk on 26200+ builds.
