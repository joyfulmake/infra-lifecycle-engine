...
**Current status:** v2.6.0.0 ready to build — Comprehensive MSIX hardening to fix persistent crash on 26200.8116 (Microsoft Surface Laptop, Error: N/A). Root cause (confirmed): WebView2 on 26200.8116 resolves `@font-face src: url()` values during CSS parse time — before any JS executes — for ALL declared fonts, even those never used by any element. The 56 @fontsource Inter woff/woff2 files bundled by Vite trigger this path and crash the native renderer. Fix in MSIX workflow: (1) delete all *.woff/*.woff2 from `msix/assets/`; (2) strip every `@font-face{…}` block from the CSS bundle; (3) strip `<link rel="icon">` and `<link rel="apple-touch-icon">` from MSIX index.html (unnecessary resource fetches at HTML parse time); (4) remove 13 web-only files from MSIX root: slides.html, aup.html, dpa.html, msa.html, sla.html, tos.html, privacy.html, manifest.json, favicon.svg, icons.svg, _redirects, _headers, 404.html. The app uses Segoe UI system font inside MSIX (forced by the existing inline script in index.html — visually identical).
