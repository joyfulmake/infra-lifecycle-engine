...
**Current status:** v2.2.0.0 ready to build — Crash at launch fix on Windows 11 26200 (Dell Latitude 5520, Lenovo ThinkPad 450s, Dell Latitude 5490). Root cause: `navigator.serviceWorker.getRegistrations()` was called in ms-appx-web: context on every launch. On fresh 26200 devices, calling ANY serviceWorker API in ms-appx-web: scheme crashes the WebView2 renderer at the native level (no JS error, process dies). The v1.9 SW-unregistration fix introduced this call to clear old SWs from upgrade installs — but on Microsoft's fresh cert lab devices there are no old SWs to clear; the API call itself is the crash. Fix: never access `navigator.serviceWorker` in ms-appx-web: context; sw.js is already excluded from the MSIX package so no SW can activate anyway. Also moved `window.onerror = () => true` to index.html inline script so it fires before any module loads.
