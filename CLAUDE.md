...
**Current status:** v2.7.0.0 ready to build — Strip @keyframes to fix crash on 26200.8655 (Lenovo ThinkPad, Surface Laptop 5). v2.6 fixed 26200.8116 (font files). 26200.8655 was always crashing — the cert lab never reached it until v2.6 passed on 26200.8116. Fix: (1) strip all `@keyframes` blocks from the CSS bundle in MSIX workflow (Tailwind ships spin/ping/bounce/pulse keyframes); (2) replace the animated splash `<style>` block in MSIX index.html with a static version (removes `@keyframes splash-pulse` from inline HTML). All CSS animations are cosmetic — app functions identically without them. Root cause hypothesis: WebView2 on 26200.8655 crashes at CSS parse time when processing `@keyframes` declarations, similar to how 26200.8116 crashed on `@font-face` declarations.
