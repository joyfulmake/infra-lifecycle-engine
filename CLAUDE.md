...
**Current status:** v2.1.0.0 ready to build — Blank screen fix on Windows 11 24H2 26100.3194 (Dell OptiPlex 7000, Surface Laptop Go). Root cause: Google Fonts render-blocking stylesheet in index.html was never actually removed despite v1.9 intent. The `<link href="https://fonts.googleapis.com/css2?...">` tag held page rendering until fonts.googleapis.com resolved — which the MS cert lab cannot reach in ms-appx-web: context, causing a permanent blank screen. Fix: @fontsource/inter CSS is now imported in main.jsx (bundled into dist/assets/), and all Google Fonts tags removed from index.html. Also re-added internetClientServer capability (was dropped from workflow despite v2.0 adding it).
