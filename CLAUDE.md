...
**Current status:** v2.4.0.0 ready to build — Crash at launch fix for Microsoft Surface Laptop (OS build 26200.8116). Root cause: v2.3 still contained `window.SpeechRecognition` access (via the SR variable in OrchestratorPanel) and Cartesia/AudioContext code paths that run at component render time — on 26200.8116 WebView2, accessing these audio/speech APIs causes a native renderer crash (Error: N/A). Fix: (1) **voice feature completely removed** — all TTS, SpeechRecognition, AudioContext, Whisper, and mic code eliminated from OrchestratorPanel.jsx (2765 → 1412 lines); cartesia.js is no longer imported anywhere; (2) `microphone` DeviceCapability removed from AppxManifest (no mic access needed); (3) crossorigin stripping in workflow now uses a fail-fast 2-pass regex that errors the CI build if any crossorigin attribute survives.
...
- v2.4.0.0 — **Voice feature removed + crash fix for 26200.8116 (Microsoft Surface Laptop).** Root cause: `window.SpeechRecognition` access and Cartesia/AudioContext code in OrchestratorPanel rendered unconditionally on every render — on 26200.8116 WebView2, accessing speech/audio APIs causes a native renderer crash with no JS error (Error: N/A in cert reports). Fix: (1) complete removal of all TTS, SpeechRecognition, AudioContext, Whisper, mic code from OrchestratorPanel.jsx (2765 → 1412 lines); no audio API is accessed at any point in the app lifecycle; (2) `cartesia.js` no longer imported anywhere — `speechSynthesis` and `AudioContext` references in that module never reach the bundle; (3) `microphone` DeviceCapability removed from AppxManifest; (4) crossorigin strip in workflow upgraded to 2-pass regex + fail-fast CI check that errors if any `crossorigin` survives.
